...
import "./../../../sass/components/_usersdashboard.scss"; // Reuse styling or adjust

function AdminModal({ onClose, onSubmit, isEdit = false, initialData = null }) {
  const [formData, setFormData] = useState({
    ...
    password: '',
    role_id: isEdit && initialData ? initialData.role_id?.toString() || '1' : '1', // Default to Admin (role_id 1)
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        } else if (isEdit && initialData && !formData.gender) {
          setFormData((prev) => ({ ...prev, gender: initialData.gender || genderResponse.data[0].id.toString() }));
        }
      } catch (error) {
