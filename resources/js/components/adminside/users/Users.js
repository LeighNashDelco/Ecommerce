  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Active Roles Response:", rolesResponse.data);
        ...
        setRoles([{ value: "all", label: "All Users", id: null }, ...fetchedRoles]);
      } catch (error) {
        console.error("Error fetching data:", error);
        console.log("Response:", error.response?.data);
        setUsers([]);
        setRoles([{ value: "all", label: "All Users", id: null }]);
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        const addedUser = {
          ...
          role_id: parseInt(newUser.get("role_id")),
          role_name: roles.find((r) => r.id === parseInt(newUser.get("role_id")))?.label || "Unknown",
          profile_img: null,
                  ...
                  currentUsers.map((user) => {
                    ...
                    return (
                      <tr key={user.id}>
                          ...
                          <img
                            src={
                              user.profile_img
                                ...
                                : `${baseImageUrl}images/pfp/default.png`
                            }
                            alt="Profile"
                        ...
                        <td>{role ? role.label : user.role_name || "N/A"}</td>
