...
const Product = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProducts([]);
        setCategories([{ value: "all", label: "All", id: null }]);
      }
    };
    fetchData();
  }, []);
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesFilter = filterValue === "all" || (productCategory && productCategory.value === filterValue);
  ...
  return (
    <div className="app">
            ...
            <table>
              ...
              <tbody>
                {currentPageProducts.length > 0 ? (
                  currentPageProducts.map((product) => {
                    ...
                    return (
                      <tr key={product.id}>
                        ...
                        <td>{category ? category.label : "N/A"}</td>
                        <td>{product.brand}</td>
                        <td>₱{product.price}</td>
                  ...
                  <tr>
                    <td colSpan="8">
                      {products.length === 0 ? "No products available" : "No products match the current filters"}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {isEditModalOpen && (
        <EditProductModal onClose={handleModalClose} onEdit={handleProductEdit} product={productToEdit} />
      )}
      {isConfirmModalOpen && (
