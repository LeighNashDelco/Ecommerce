...
function EditProductModal({ onClose, onEdit, product }) {
  ...
  // Helper function to parse price string with commas
  const parsePrice = (price) => {
    if (typeof price === "string") {
      return Number(price.replace(/,/g, "")); // Remove commas and convert to number
    }
    return Number(price); // If already a number, just convert
  };
  ...
  const [formData, setFormData] = useState({
    ...
    setPrice: parsePrice(product.price), // Parse price correctly
    setStock: Number(product.quantity),
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((prevData) => ({
      ...prevData,
      [name]: name === "setPrice" || name === "setStock" ? Number(value) : value,
    }));
  };
