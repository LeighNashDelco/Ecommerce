...
import { IconTrash, IconEdit, IconRefresh, IconSearch, IconPlus, IconEye, IconArchive } from "@tabler/icons-react";
...
const Roles = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [activeResponse, archivedResponse, rolesFilterResponse] = await Promise.all([
          axios.get("http://127.0.0.1:8000/api/roleslist", config),
          axios.get("http://127.0.0.1:8000/api/roleslist/archived", config),
          axios.get("http://127.0.0.1:8000/api/roles/activeroles", config), // Fetch active roles for filter
        ]);
        ...
        const fetchedFilterOptions = rolesFilterResponse.data.map((role) => ({
          value: role.role_name.toLowerCase(),
          label: role.role_name,
        }));
        setFilterOptions([{ value: "all", label: "All Roles" }, ...fetchedFilterOptions]);

        console.log("Active Roles Response:", activeResponse.data);
        console.log("Archived Roles Response:", archivedResponse.data);
        console.log("Filter Roles Response:", rolesFilterResponse.data);
      } catch (error) {
        console.error("Error fetching roles:", error);
        console.log("Response:", error.response?.data);
        setRoles([]);
        setFilterOptions([{ value: "all", label: "All Roles" }]); // Fallback
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredRoles = roles.filter((role) => {
    ...
    const matchesFilter = filterValue === "all" || role.role_name.toLowerCase() === filterValue;
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        setFilterOptions((prevOptions) => [
          ...prevOptions,
          { value: addedRole.role_name.toLowerCase(), label: addedRole.role_name },
      ...
      if (response.status === 200) {
        ...
        setFilterOptions((prevOptions) =>
          prevOptions.map((option) =>
            option.value === roleToEdit.role_name.toLowerCase()
              ? { value: editedRole.role_name.toLowerCase(), label: editedRole.role_name }
              : option
          )
        ); // Update filter options dynamically
        setIsModalOpen(false);
